...
- 계정별 주간 사용률, 남은 비율, 초기화 시각 표시
- 리셋 크레딧 개수와 서버가 제공하는 가장 빠른 소멸 시각 표시
- 현재 머신의 기본 Codex 계정과 앱에 추가한 여러 ChatGPT 계정 동시 확인
- 터미널 프롬프트 모양의 메뉴 바 아이콘에 기본 계정의 남은 비율 표시
...
Node.js와 Codex CLI는 필요하지 않습니다. Codex CLI에 이미 로그인된 Mac에서는 해당 로그인을 기본 계정으로 그대로 사용합니다. 다른 계정은 앱의 **계정 추가**에서 로그인할 수 있으며, 각 계정의 인증 정보는 서로 분리된 앱 전용 `CODEX_HOME`에 저장됩니다.

추가 계정의 저장 위치는 macOS가 제공하는 Application Support 아래의 `Codex Usage/Accounts/{UUID}/CodexHome`입니다. App Store 빌드에서는 앱 샌드박스 컨테이너 안에 생성됩니다. 앱은 인증 토큰 내용을 직접 파싱하거나 계정 간에 복사하지 않습니다.
